...
export const subscriptionRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/checkout", { preHandler: requireAuth }, async (request, reply) => {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: successUrl ?? `${process.env.APP_URL ?? "https://ouden.cc"}/subscription?success=1`,
      cancel_url: cancelUrl ?? `${process.env.APP_URL ?? "https://ouden.cc"}/subscription`,
      subscription_data: {
  ...
  app.post("/portal", { preHandler: requireAuth }, async (request, reply) => {
    ...
    const session = await stripe.billingPortal.sessions.create({
      ...
      return_url: `${process.env.APP_URL ?? "https://ouden.cc"}/subscription`,
    });
